...
You should install all the dependencies with `pip` via the extras in `pyproject.toml`:
...
dora run -d -f 81de367c hdemucs_mlx.channels=32  # start from the config of XP 81de367c but change some hyper-params.
...
`demucs_mlx/remote/mdx.yaml` to `./release_models/my_bag.yaml`. You can then edit the list of models (all models used should have been exported first) and the weights per source and model (list of list, outer list is over models, inner list is over sources). You can then use your bag of model as
...
(you can retrieve it with `demucs_mlx.pretrained.get_model(SIG)`). The bag of models are available
by doing `demucs_mlx.pretrained.get_model(NAME)` with `NAME` begin either `mdx` (for Track A) or `mdx_extra`
(for Track B).
...
The experiments are provided in the grids [repro.py](../demucs_mlx/grids/repro.py) and
[repro_ft._py](../demucs_mlx/grids/repro_ft.py) for the fine tuning on the realistic mix datasets.
